...
static mx_status_t xhci_start_transfer_locked(xhci_t* xhci, xhci_endpoint_t* ep, iotxn_t* txn) {
    ...
    if (txn->length > 0) {
        iotxn_phys_iter_init(&state->phys_iter, txn);
        mx_paddr_t dummy_paddr;
        size_t dummy_length;
        while (iotxn_phys_iter_next(&state->phys_iter, XHCI_MAX_DATA_BUFFER, &dummy_paddr,
               &dummy_length)) {
            state->packet_count++;
        }
    }
...
static mx_status_t xhci_continue_transfer_locked(xhci_t* xhci, xhci_endpoint_t* ep, iotxn_t* txn) {
    ...
    while (free_trbs > 0 && iotxn_phys_iter_next(&state->phys_iter, XHCI_MAX_DATA_BUFFER, &paddr,
                                                 &transfer_size)) {
        xhci_trb_t* trb = ring->current;
        xhci_clear_trb(trb);
